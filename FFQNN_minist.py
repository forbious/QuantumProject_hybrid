...
class HybridFunction(Function):
    ...
    def backward(ctx, grad_output):
        ...
        return torch.tensor([gradients]).float().to(device) * grad_output.float(), None, None
...
class Net_sim(nn.Module):
    def __init__(self):
        super(Net_sim, self).__init__()
...
for epoch in range(epochs):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        # Forward pass
        output = model_sim(data.to(device))
        # Calculating loss
    ...
    loss_list_sim.append(sum(total_loss)/len(total_loss))
    print('Training [{:.0f}%]\tLoss: {:.4f}'.format(
        100. * (epoch + 1) / epochs, loss_list_sim[-1]))
...
with torch.no_grad():
    ...
    print('Performance on sim test data:\n\tLoss: {:.4f}\n\tAccuracy: {:.1f}%'.format(
